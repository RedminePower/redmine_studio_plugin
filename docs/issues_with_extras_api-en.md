...
The following 4 fields are added to the standard issue response.
...
Includes all fields of the standard `GET /issues/:id.json` plus the 4 fields above.
...
`last_updated_by` uses the same `id` / `name` attribute format as `author` / `assigned_to`.

## Relationship to the Standard Endpoint

- `issues_with_extras/*.api.rsb` is a copy of Redmine core's `issues/*.api.rsb` with the 4 fields added. Because the core view is not overridden, the standard `GET /issues` does not return these fields (zero side effects).
